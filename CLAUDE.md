...
content/
  ...
  kutaisi-project/  # self-contained Kutaisi land-sale page, sourced from the
                    # separate ~/projects/kutaisi-land repo. Refresh it with
                    # `python3 sync_kutaisi.py` (rewrites URLs + injects the
                    # analytics beacon), then rebuild. Language folders are
                    # /, ge/, ru/ — hence KUTAISI_PATHS in build.py. The brass
                    # "Kutaisi Project" header pill links to it. The source
                    # repo's own GitHub Pages is disabled — this is the only
                    # live copy.
...
2. Create `content/projects/<new-slug>/project.md`:
   ...
   The grid sorts by `year`, newest first (a range like `2015–2018`
   sorts by its end); `order` only breaks ties within the same year
   (lower = earlier). To retire a project, `git rm` its content dir and
   docs/img dir and add its slug to `SLUG_REDIRECTS` in build.py so the
