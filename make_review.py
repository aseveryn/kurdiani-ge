...
def main():
    ...
    texts = {}
    for code in ("en", "ka", "ru"):
        path = os.path.join(ROOT, "content", "home", f"intro.{code}.html")
        texts[code] = strip_html(io.open(path, encoding="utf-8").read()) \
            if os.path.exists(path) else ""
    ...
    out.write(
        ...
        "- **6 hotel projects** — counted from the six hotels on the site. It says "
        "**projects** deliberately: five of the six are titled 'Project', so they "
        "read as designs rather than finished hotels, and saying '6 hotels' would "
        "claim more than the site shows. If they were built, say so and the label "
        "becomes '6 hotels'. The count also leaves out the Hotel 'Sachveno' "
        "interior, which the site files under interiors. **How many hotels have "
        "you actually designed, and how many were built?** Over forty years it is "
        "very likely more than six.\n\n"
    ...
    texts = {}
    for code in ("en", "ka", "ru"):
        path = os.path.join(ROOT, "content", "home", f"stats.{code}.html")
        texts[code] = strip_html(io.open(path, encoding="utf-8").read()) \
            if os.path.exists(path) else ""
    multiline_block(out, "The numbers", "Shown in large type under the introduction.", texts)
